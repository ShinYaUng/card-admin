...
namespace NueGames.NueDeck.Scripts.Card
{
    public class CardBase : MonoBehaviour, I2DTooltipTarget, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public virtual void SetCard(CardData targetProfile, bool isPlayable = true)
        {
            ...
            var desc = CardData.MyDescription;
            if (!string.IsNullOrEmpty(desc) && desc.Contains("{totalDamage}"))
                desc = desc.Replace("{totalDamage}", CalculateTotalDamagePreview().ToString());
        ...
        protected virtual void SpendMana(int value)
        {
            ...
            // ⬇️ ถ้ามี FirstCardFree และยังไม่ใช้ใน combat นี้ ให้ฟรี
            if (pgd.HasRelic(NueGames.NueDeck.Scripts.Enums.RelicType.FirstCardFree) &&
                !pgd.FirstCardFreeUsedThisCombat)
            {
                cost = 0;
                pgd.FirstCardFreeUsedThisCombat = true;
            }

            pgd.CurrentMana -= cost;

            // อัปเดต UI พลังงาน หากต้องการ
        ...
        #endregion
        [SerializeField] private CharacterBase ownerCharacter;
        ...
        private float GetBaseCardDamage()
        {
            var attack = CardData.CardActionDataList
                .FirstOrDefault(a => a.CardActionType == NueGames.NueDeck.Scripts.Enums.CardActionType.Attack);
            return attack != null ? attack.ActionValue : 0f;
        }

        private int GetOwnerRunAttack(CharacterBase c)
        {
            if (c is AllyBase ally)              // ค่าที่เปลี่ยนหลังอัพแต้มในรอบเล่น
                return ally.RunBaseAttackBonus;  // <<< ใช้ค่านี้เป็นหลัก
            return (c?.CharacterData != null) ? c.CharacterData.BaseAttackPower : 0;
        }

        private int CalculateTotalDamagePreview()
        {
            var baseDmg = GetBaseCardDamage(); // ดึง ActionValue ของแอ็กชัน Attack
            if (ownerCharacter == null) return Mathf.RoundToInt(baseDmg);

            int str = 0;
            if (ownerCharacter.CharacterStats.StatusDict.TryGetValue(StatusType.Strength, out var s))
                str = s.StatusValue;

            var runAtk = GetOwnerRunAttack(ownerCharacter);
            return Mathf.RoundToInt(baseDmg + str + runAtk);
        }
    }
}