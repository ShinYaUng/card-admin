...
namespace NueGames.NueDeck.Scripts.Collection
{
    public class HandController : MonoBehaviour
    {
        ...
        public void AddCardToHand(CardBase card, int index = -1)
        {
            ...
            {
                card.SetOwner(CombatManager.CurrentMainAlly); // ผูกเจ้าของการ์ด = ผู้เล่นหลัก
                card.UpdateCardText();                         // รีเฟรช description ให้แทน {totalDamage}
            }
        }
        ...
        {
            foreach (var c in hand) c.UpdateCardText();
        }
