...
import { createCard, updateCard } from "./api.js";
...
export default function CardForm({ onSaved, editData, onCancel }) {
    ...
    <div style={{ background: "#f8f8f8", padding: 12, borderRadius: 8 }}>
      ...
      <form onSubmit={submit}>
        ...
        <div style={{ marginTop: 8 }}>
          <label>
            <input type="checkbox" checked={form.usableWithoutTarget} onChange={e => setField("usableWithoutTarget", e.target.checked)} /> usableWithoutTarget
          </label>{" "}
          <label>
            <input type="checkbox" checked={form.exhaustAfterPlay} onChange={e => setField("exhaustAfterPlay", e.target.checked)} /> exhaustAfterPlay
          </label>
        </div>
        ...
        <fieldset style={{ marginTop: 12 }}>
          ...
          {form.desc.map((d, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 100px 100px 1fr 80px", gap: 8, marginBottom: 6 }}>
              ...
              <label>
                <input type="checkbox" checked={d.useModifier} onChange={e => updDesc(i, { ...d, useModifier: e.target.checked })} /> mod
              </label>
  ...
  function updAction(i, v) {
    setForm(f => ({ ...f, actions: f.actions.map((x, idx) => (idx === i ? v : x)) }));
  }
  function delAction(i) {
    setForm(f => ({ ...f, actions: f.actions.filter((_, idx) => idx !== i) }));
  }
  function updDesc(i, v) {
    setForm(f => ({ ...f, desc: f.desc.map((x, idx) => (idx === i ? v : x)) }));
  }
  function delDesc(i) {
    setForm(f => ({ ...f, desc: f.desc.filter((_, idx) => idx !== i) }));
  }
}
